#mpyexo
"""High-resolution framebuffer demo with centered welcome text and animated shapes."""
...
def _load_console_api():
    ...
    if not callable(create) or not callable(fill_rect) or not callable(present) or not callable(sleep_hz):
        raise RuntimeError('framebuffer functions unavailable')
...
def _draw_text_centered(surface, fill_rect, text, color, box_y, box_h):
    ...
    while index < len(text):
        char_x = start_x + index * (char_w + spacing)
        _draw_char(surface, fill_rect, text[index], char_x, start_y, color)
        index += 1
...
def run_demo():
    ...
    while frame < FRAMES:
        ...
        # dotted background pattern for higher-detail look
        dot = 0
        while dot < width:
